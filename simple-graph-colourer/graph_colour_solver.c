        ...
        int domain_num_words)
{
    ...
    for (int i=0; i<original_g->n; i++)
        if (bitset_popcount(available_classes_bitset + i * domain_num_words, domain_num_words) == 1)
    ...
    while (unit_orig_v_stack_sz) {
        ...
        solution_colour_vtx(C, orig_v, colour);
        clear_bitset(available_classes_bitset + orig_v * domain_num_words, domain_num_words);
    ...
    while (!bitset_empty(domain_copy, domain_num_words)) {
        ...
        clear_bitset(new_available_classes_bitset + best_orig_v * domain_num_words, domain_num_words);
        ...
        solution_colour_vtx(C, best_orig_v, orig_colour);
        expand(original_g, C, incumbent, level+1, expand_call_count,
                quiet, num_colours, new_available_classes_bitset, domain_num_words);
        solution_pop_vtx(C);

        if (incumbent->size == original_g->n)
            break;
        ...
        bool quiet, struct Solution *incumbent, int num_colours)
{
    ...
    expand(original_g, &C, incumbent, 0, expand_call_count,
            quiet, num_colours, available_classes_bitset, domain_num_words);
